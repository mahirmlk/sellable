...
const statusConfig: Record<TransactionStatus, { label: string; color: string; bg: string }> = {
  QUOTED: { label: "QUOTED", color: "text-[var(--bb-grey-1)]", bg: "bg-[var(--bb-panel-2)]" },
  ...
  DENIED: { label: "DENIED", color: "text-red-400", bg: "bg-red-400/10" },
  NEEDS_HUMAN_APPROVAL: { label: "NEEDS APPROVAL", color: "text-amber-400", bg: "bg-amber-400/10" },
...
export function StatusBadge({ status }: { status: TransactionStatus }) {
  const cfg = statusConfig[status] || statusConfig.QUOTED;
  return (
    <span
